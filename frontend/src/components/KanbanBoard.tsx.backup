...
const KanbanBoard: React.FC = () => {
  ...
  useEffect(() => {
    // Tâches initiales complètes avec toutes les informations
    const initialTasks: Task[] = [
      ...
      },
      {
        ...
        description: 'Rédiger la documentation API et guide d\'installation',
        priority: 'medium',
        labels: ['documentation', 'api'],
        status: 'todo',
        assignee: 'Sophie Bernard',
        dueDate: format(new Date(Date.now() + 86400000 * 5), 'dd/MM/yyyy'),
        createdAt: format(new Date(Date.now() - 86400000), 'dd/MM/yyyy HH:mm')
      }
    ];
